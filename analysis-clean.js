...
class ChessAnalysis {
    ...
    async analyzeCurrentPosition() {
        if (this.isAnalyzing || !this.stockfishReady) return;
        ...
        try {
            ...
            const result = await this.stockfish.evaluate(fen, 12);
            ...
            this.updateStatus(`Analysis complete! Best move: ${result.bestMove}`, 'ready');
        } catch (error) {
            console.error('Analysis failed:', error);
            this.updateStatus('Analysis failed - Stockfish error', 'error');
        } finally {
            this.isAnalyzing = false;
        }
    }
